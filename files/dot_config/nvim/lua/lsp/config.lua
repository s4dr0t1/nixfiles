...
function lsp_keymaps(bufnr)
	...
	set_lsp_key(bufnr, "n", "[d", '<cmd>lua vim.diagnostic.goto_prev({ border = "rounded" })<CR>', opts)
	set_lsp_key(
		bufnr,
		"n",
		"gl",
	...
	set_lsp_key(bufnr, "n", "]d", '<cmd>lua vim.diagnostic.goto_next({ border = "rounded" })<CR>', opts)
	set_lsp_key(bufnr, "n", "<leader>q", "<cmd>lua vim.diagnostic.setloclist()<CR>", opts)
